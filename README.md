...
## 简介
[Vim](https://www.vim.org) 是一款可高度定制化的文本编辑器，可以用来高效地生成、修改任何形式的文本文件。
Vim 是一个开源项目，源码公开在 [Github](https://github.com/vim/vim)。

Windows 用户可以从此下载最新的每日构建版： [vim-win32-installer](https://github.com/vim/vim-win32-installer/releases)。
...
打开 Vim 时的默认模式是 Normal 模式，在这一模式下，按键不能直接将字符插入至文件，而是执行各种命令。比如 `h` 按键不是输入字符 h，而是将光标
向左移动一个字符位置。
...
普通模式是 Vim 的默认模式，任何模式下通过 `<ESC>` 按键都可以切换到普通模式。
